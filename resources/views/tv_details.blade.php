...
<body>
    <div class="w-full h-screen flex flex-col relative">
        @php
            ...
            if($tv_data){
                $backdropPath = "{$image_url}/original{$tv_data->backdrop_path}";
                $oriDt = $tv_data->first_air_date;
                $timeStamp = strtotime($oriDt);
                $tvShowYear = date("Y", $timeStamp);
                $rating = round((float)($tv_data->vote_average*10));
                $title = $tv_data->name;
                ...
                if($tv_data->tagline){
                    ...
                    $tagline = $tv_data->overview;
                }

                if($tv_data->episode_run_time){
                    $runtime = $tv_data->episode_run_time[0];
                    $duration = "{$runtime}m/eps";
                }
            }
            //32 pixel (2 * pi * r)
            ...
            if(isset($tv_data->videos->results)){
                foreach ($tv_data->videos->results as $item) {
        ...
        <div class="w-full bg-transparent h-[96px] drop-shadow-lg flex flex-row items-center z-10">
            ...
            <div class="w-1/3 flex flex-row justify-end pr-10">
                <a href="/search" class="group">
                    <svg style="color: rgb(24, 24, 22);" xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="w-full h-full z-10 flex flex-col justify-center px-20">
            ...
            <div class="flex flex-row mt-4 items-center">
                ...
                <div class="w-20 h-20 rounded-full flex items-center justify-center mr-4" style="background: #00304D;">
                    <svg class="-rotate-90 w-20 h-20">
                        <circle style="color: #004F80;" stroke-width = "8" stroke="currentColor" fill="transparent" r="32" cx="40" cy="40"/>
                        <circle style="color: #6FCF87;" stroke-width = "8" stroke-dasharray="{{$circumference}}" stroke-dashoffset="{{$progressRating}}" stroke-linecap="round" stroke="currentColor" fill="transparent" r="32" cx="40" cy="40"/>
                    </svg>
                    <span class="absolute font-inter font-bold text-xl text-white">{{$rating}}%</span>
                </div>

                {{-- movie year --}}
                <span class="font-inter text-xl text-white bg-transparent rounded-md border border-white p-2 mr-4">{{$tvShowYear}}</span>
            ...
            {{-- play trailer --}}
            @if ($trailerId)
            <button class="w-fit bg-customColors-500 text-white pl-4 pr-6 py-3 mt-5 font-inter text-xl flex flex-row rounded-lg items-center hover:drop-shadow-lg duration-200" onclick="showTrailer(true)">
                <svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-play-fill" viewBox="0 0 16 16"> <path d="m11.596 8.697-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393z" fill="white"></path> </svg>
        ...
        <div class="absolute z-10 w-full h-screen p-20 bg-black flex flex-col" id="trailerWrapper">
            <button class="ml-auto group mb-4" onclick="showTrailer(false)">
                <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48">
                  <path d="m12.45 37.65-2.1-2.1L21.9 24 10.35 12.45l2.1-2.1L24 21.9l11.55-11.55 2.1 2.1L26.1 24l11.55 11.55-2.1 2.1L24 26.1Z" class="fill-white group-hover:fill-customColors-500 duration-200"/>
                </svg>
              </button>

            <iframe id="youtubeVideo" class="w-full h-full" src="https://www.youtube.com/embed/{{$trailerId}}?enablejsapi=1" title="{{$title}}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
        </div>
    </div>
