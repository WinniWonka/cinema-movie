...
<body>
    <div class="w-full h-auto min-h-screen flex flex-col">
         ...
         <div class="w-full pl-28 pr-10" id="loadMore">
            <button class="w-full mb-10 bg-customColors-500 text-white p-4 font-inter font-bold rounded-xl uppercase drop-shadow-lg" onclick="loadMore()">Load More ...</button>
         </div>
    ...
    <script>
        ...
        function loadMore(){
            $.ajax({
                url:`${baseUrl}/discover/tv?page=${++page}&sort_by=${sortBy}&api_key=${apiKey}&vote_count.gte=${minimal_voter}`,
                type:"get",
                beforeSend: function(){
                    //show loader
                    $("#autoLoad").show();
            ...
            .done(function (response){
                ...
                if(response.results){
                    ...
                    response.results.forEach(item => {
                        let oriDt = item.first_air_date;
                        let timeStamp = new Date(oriDt);
                        let tvShowYear = timeStamp.getFullYear();

                        let tvShowId = item.id;
                        let tvShowTitle = item.name;
                        let tvShowRate = Math.round(item.vote_average*10);
                        let tvShowImg = `${imgBaseUrl}/w500${item.poster_path}`; 
